...
#define LABEL_NALE_TXT_PATH "../../weights/plate.txt"
                   ...
                   int32_t zp, float scale)
{
  ...
  */
 /* 16 结构
 x  y  w  h  conf, pt1x pt1y pt2x pt2y pt3x pt3y pt4x pt4y, cls0, cls1, cls2
 0        3   4    5                                        13    14    15
 */
  for (int a = 0; a < 3; a++)
  {
      for (int i = 0; i < grid_h; i++)
      {
        ...
        {
          int8_t box_confidence = input[PROP_BOX_SIZE *(a * grid_len  + i * grid_w  + j ) + 4];
          if (box_confidence >= thres_i8)
          {
            ...
            //calculate landmarks
            for (int z = 0; z < 4; z ++){
              landmark_x[z] = deqnt_affine_to_f32(in_ptr[5 + z *2], zp, scale) ;
              landmark_y[z] = deqnt_affine_to_f32(in_ptr[6 + z *2], zp, scale) ;
            ...
            int8_t maxClassProbs = in_ptr[13];  //cls0的位置
            int maxClassId = 0;
            for (int k = 1; k < OBJ_CLASS_NUM; ++k)
            {
              int8_t prob = in_ptr[13 + k];
                 ...
                 std::vector<float> &qnt_scales, detect_result_group_t *group)
{
  ...
  for (int i = 0; i < validCount; ++i)
  {
    ...
    for(int i =0; i < 4; i ++){
        float x = landmarks[n * 8 + i * 2] - pads.left;
        float y = landmarks[n * 8 + i * 2 + 1] - pads.top;
        group->results[last_count].landmarks[i * 2] = (int) (clamp(x, 0, model_in_w) / scale_w) ;
        group->results[last_count].landmarks[i * 2 + 1] = (int) (clamp(y, 0, model_in_h) / scale_h) ;
    }
