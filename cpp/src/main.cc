...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < detect_result_group.count; i++)
  {
    ...
    rectangle(orig_img, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(256, 128, 0, 128), 3);
    putText(orig_img, text, cv::Point(x1, y1), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 255), 2);

    // 过滤车辆的landmark点
    if (det_result->class_id==2){
      continue;
    } 
    ...
    cv::Scalar(255, 0, 0, 255), // 红色
    cv::Scalar(0, 0, 255, 255), // 蓝色
    cv::Scalar(255, 255, 0, 255) // 黄色
    }; 
    ...
    for(int i =0; i < 4 ; i ++)
    {
      ...
      circle(orig_img, cv::Point(x, y), 2, colors[i], 2);
    } 
  }
