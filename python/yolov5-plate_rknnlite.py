...
class YOLOv5:
    ...
    def __call__(self, img_list):
        ...
        det = results[0]
        print(f"Find {det.shape[0]} objects")
        res_img = draw_numpy(img_list[0], det[:,:4], masks=None, classes_ids=det[:, 13], conf_scores=det[:, 4], landmarks=det[:, 5:13])
...
def draw_numpy(image, boxes, masks=None, classes_ids=None, conf_scores=None, landmarks=None):
    for idx in range(len(boxes)):
        ...
        if classes_ids is not None and conf_scores is not None:
            ...
            cv2.putText(image, PLATE_CLASSES[int(classes_ids[idx])] + ':' + str(round(conf_scores[idx], 2)),
                        (x1, y1 - 5),
        ...
        for idx, landmark in enumerate(landmarks):
            # 过滤车辆的landmark
            if int(classes_ids[idx]) == 2:
                continue
            for i in range(4):
                point_x = int(landmark[2 * i])
                point_y = int(landmark[2 * i + 1])
                cv2.circle(image, (point_x, point_y), 4, clors[i], -1)
...
def argsparser():
    ...
    parser.add_argument('--model', type=str, default='../weights/car_plate_detect_1x3x80x80x16-640-640_rm_transpose_rk3588.rknn', help='path of model')
...
if __name__ == '__main__':
    args = argsparser()
    yolov5_plate = YOLOv5(args)
    result = yolov5_plate([cv2.imread(args.input)])
