    ...
    def  __call__(self, preds_batch, org_size_batch, ratios_batch, txy_batch):
        ...
        for det, (org_w, org_h), ratio, (tx1, ty1) in zip(outs, org_size_batch, ratios_batch, txy_batch):
            if len(det):
                ...
                landmarks = det[:, 5:13]
                landmarks[:, [0, 2, 4, 6]] -= tx1  # x padding
                landmarks[:, [1, 3, 5, 7]] -= ty1  # y padding
                landmarks[:, [0, 2, 4, 6]] /= ratio[0]   # x padding
                landmarks[:, [1, 3, 5, 7]] /= ratio[1]  # y padding
                landmarks[:, [0, 2, 4, 6]] = landmarks[:, [0, 2, 4, 6]].clip(0, org_w -1)
                landmarks[:, [1, 3, 5, 7]] = landmarks[:, [1, 3, 5, 7]].clip(0, org_h - 1)
                det[:, 5:13] = landmarks.round()
...
class pseudo_torch_nms:
    ...
    def non_max_suppression(self,
        ...
        nc = prediction.shape[2] - 5 - 8  # number of classes, and 8 landmark points
        ...
        output = [np.zeros((0, 16))] * prediction.shape[0]    # add 8 landmarks + 3 labels
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            if multi_label:
                i, j = np.where(x[:, 13:] > conf_thres)
                x = np.concatenate([box[i], x[i, j + 13, None], x[i, 5:13].astype(np.float32), j.astype(np.float32)[:, None]], 1)
            else:  # best class only
                conf = x[:, 13:].max(1, keepdims=True)
                j_argmax = x[:, 13:].argmax(1)
                j = j_argmax if j_argmax.shape == x[:, 13:].shape else \
            ...
            if not n:  # no boxes
                ...
                x = x[x_argsort]
            # Batched NMS
            c = x[:, 13:] * (0 if agnostic else max_wh)  # classes
